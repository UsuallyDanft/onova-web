                  ...
                  <div className="cart-sidebar-item-info">
                    ...
                    <div className="cart-sidebar-item-controls">
                      <div className="cart-sidebar-quantity">
                        <button onClick={() => handleQuantityChange(item.id, item.quantity - 1, item.stock)} disabled={item.quantity <= 1}>
                          <Minus size={12} />
                        </button>
                        <span>{item.quantity}</span>
                        <button onClick={() => handleQuantityChange(item.id, item.quantity + 1, item.stock)} disabled={item.quantity >= item.stock}>
                          <Plus size={12} />
                        </button>
                      </div>
                      <button onClick={() => handleRemoveItem(item.id)} className="cart-sidebar-remove">
                        <Trash2 size={14} />
                      </button>
                    </div>
        ...
        {items.length > 0 && (
          <div className="cart-sidebar-footer">
            ...
            <div className="cart-sidebar-actions">
              <button onClick={onClose} className="cart-sidebar-continue">Continuar</button>
              <button onClick={handleCheckout} className="cart-sidebar-checkout">Pagar</button>
            </div>
          </div>
        )}
      </div>
